...
export default function FacultyCourseAssignments() {
    const { cid } = useParams();
    ...
    const fetchCourse = async()=>{
      try {
        const response = await axios.get(`${BackendURLS.Faculty}/getcourse?id=${cid}`);
            ...
            <TableBody emptyContent={"No Assignments Found...!"}>
              {currentAssignments.map((assignment, index) => (
                <TableRow key={index} className="hover-row">
                  ...
                  <TableCell>
                    <Button color="success" variant="shadow" className="hover-3d" as={Link} to={`/faculty/facultyassignmentsubmissions/${assignment.assignmentId}`}>
                      View Submissions
                    </Button>
                  </TableCell>
                </TableRow>
