...
export default function FacultyProfile() {
    ...
    const getUpdatedFaculty = async () => {
      try {
        const response = await axios.get(
          `${BackendURLS.Faculty}/viewfaculty?id=${faculty.fid}`
        );
        setFaculty(response.data);
        sessionStorage.setItem("faculty", JSON.stringify(response.data)); // Update with response data
      } catch (error) {
    ...
    const updateProfilePicture = async () => {
      ...
      formData.append("id", faculty.fid);
      ...
      try {
        ...
        const response = await axios.put(
          `${BackendURLS.Faculty}/updatefacultyprofile`,
          formData,
          {
            headers: {
    ...
    const changePassword = async()=>{
      ...
      try {
        const response = await axios.put(`${BackendURLS.Faculty}/changepassword`,data);
          ...
          <div className="profile-page__card">
            <div className="profile-page__header">
              <div className="profile-page__image-container">
                <img
                  src={`data:image/jpeg;base64,${faculty.fprofile}`}
                  alt={`${faculty.fname}'s profile`}
                  className="profile-page__image"
                />
              </div>
              <h1 className="profile-page__name">{faculty.fname}</h1>
              <p className="profile-page__department">{faculty.fdepartment}</p>
            </div>
            <div className="profile-page__info">
              <div className="profile-page__field">
                <span className="profile-page__label">Student ID:</span>
                <span className="profile-page__value">{faculty.fid}</span>
              </div>
              <div className="profile-page__field">
                <span className="profile-page__label">Email:</span>
                <span className="profile-page__value">{faculty.femail}</span>
              </div>
              <div className="profile-page__field">
                <span className="profile-page__label">Contact:</span>
                <span className="profile-page__value">{faculty.fcontact}</span>
              </div>
              <div className="profile-page__field">
                <span className="profile-page__label">Date of Birth:</span>
                <span className="profile-page__value">{faculty.dob}</span>
              </div>
              <div className="profile-page__field">
                <span className="profile-page__label">Age:</span>
                <span className="profile-page__value">{faculty.age}</span>
              </div>
