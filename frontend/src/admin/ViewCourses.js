...
export default function ViewCourses() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const coursesWithCounts = await Promise.all(response.data.map(async (course) => {
  ...
  const filteredCourses = courses.filter(course => {
    ...
    const courseSemester = course.courseSemester ? course.courseSemester.toLowerCase() : '';
    const instructorId = course.courseInstructorID ? course.courseInstructorID.toLowerCase() : '';
    const instructorName = course.courseInstructorName ? course.courseInstructorName.toLowerCase() : '';
    const academicYear = course.academicyear ? course.academicyear.toLowerCase() : '';
    const department = course.courseDepartment ? course.courseDepartment.toLowerCase() : '';
      ...
      <div>
        ...
        <div className="overflow-x-auto rounded-2xl bg-white mt-4 mx-5">
          <table className="table">
            ...
            <thead>
              <tr>
                <th align="center" className="text-black">COURSE ID</th>
                <th align="center" className="text-black">COURSE CODE</th>
                <th align="center" className="text-black">COURSE NAME</th>
                <th align="center" className="text-black">COURSE DEPARTMENT</th>
                <th align="center" className="text-black">COURSE SEMESTER</th>
                <th align="center" className="text-black">INSTRUCTOR ID</th>
                <th align="center" className="text-black">INSTRUCTOR NAME</th>
                <th align="center" className="text-black">ACADEMIC YEAR</th>
                <th align="center" className="text-black">COURSE REGISTERED COUNT</th>
                <th align="center" className="text-black">Actions</th>
              </tr>
            </thead>
            <tbody>
              {currentCourses.length > 0 ? (
                currentCourses.map(course => (
                  <tr key={course.cid}>
                    ...
                    <td align="center">
                      <Button color="secondary" variant="shadow" className="transform transition-all duration-300 ease-in-out 
             hover:scale-110 hover:shadow-2xl hover:translate-y-1"
>View Registered Students</Button>
                      <br/><br/>
                      <Button color="secondary" variant="shadow" className="transform transition-all duration-300 ease-in-out 
             hover:scale-110 hover:shadow-2xl hover:translate-y-1"
>View Registered Faculty</Button>

                  <Button onClick={()=>navigate(`/admin/uploadassignment/${course.cid}`)} >Upload Assignment</Button>
                    </td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan={8} align="center">No course records found!</td>
                </tr>
              )}
            </tbody>
          </table>
